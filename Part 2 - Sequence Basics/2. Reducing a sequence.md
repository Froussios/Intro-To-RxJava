...
`filter` - принимает функцию предикат которая принимает нулевое решение относительно каждого испущенного значения. Если  решение ложно(`false`) элемент отбрасывается из фильтруемой последовательности.
