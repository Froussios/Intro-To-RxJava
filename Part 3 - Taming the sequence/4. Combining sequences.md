...
`concat` seems conceptually similar to `flatMap`. The key difference is that `flatMap` will interleave the sequences if they are emitting values in parallel, while `concat` will wait for the current sequence to complete before subscribing to the next observable.
...
Observable.combineLatest(
		Observable.interval(100, TimeUnit.MILLISECONDS)
			.doOnEach(i -> System.out.println("Left emits")),
		Observable.interval(150, TimeUnit.MILLISECONDS)
			.doOnEach(i -> System.out.println("Right emits")),
		(i1,i2) -> i1 + " - " + i2
	)
	.take(6)
	.subscribe(System.out::println);
