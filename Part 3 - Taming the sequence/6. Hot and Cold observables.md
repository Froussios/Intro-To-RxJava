...
The two subscribers don't receive the same value at the same time, even though they are both subscribed to the same observable. They do see the same sequence, only each of them sees it as having begun when they subscribed. 
...
The `share` method is an alias for `Observable.publish().ConnectableObservable.refCount()`. It allows your subscribers to share a subscription. The subscription is kept for as long as there are subscribers.
