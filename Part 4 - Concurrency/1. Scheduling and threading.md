...
```java
Scheduler.Worker worker = sheduler.createWorker();
worker.schedule(
...
```java
Scheduler sheduler = Schedulers.newThread();
...
`Scheduler.Worker` extends `Subscription`. Calling the `unsubscribe` method on a worker will result on the queue being emptied and all pending tasks to be cancelled. We can see that by modifying out previous example
...
```java
Scheduler sheduler = Schedulers.immediate();
Scheduler.Worker worker = sheduler.createWorker();
worker.schedule(() -> {
...
```
Scheduler sheduler = Schedulers.trampoline();
Scheduler.Worker worker = sheduler.createWorker();
worker.schedule(() -> {
	System.out.println("Start");
	worker.schedule(() -> System.out.println("Inner"));
